...
7. Check upstream licenses before reusing code or artwork. Preserve copyright, license notices, and required credit for approved reuse.
...
- Use four-space indentation for PowerShell and two spaces for YAML and JSON. Preserve existing SVG formatting; regenerate banners through the [shared banner workflow](docs/development.md#maintain-the-shared-banner-layout). Follow `.gitattributes`: CRLF for `.ps1`, LF for other maintained text.
...
- Keep explicit front matter on contributor and conduct pages so Jekyll renders them, and retain the examples index's `/examples/` permalink. Use relative `.md` links in Markdown and final `.html` URLs in Pages navigation.
