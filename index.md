...
> **Current version: 1.0.0.** This baseline retains the reviewed deployment behavior
> and consistent package instructions. The workflow publishes a prerelease candidate.
> Downloaded-asset verification and maintainer approval precede promotion to a
> regular release marked latest. Live ConfigMgr, WinPE, and SMB testing was not executed.
> This site follows `main`; the [v1.0.0 release](https://github.com/vartaxe/ConfigMgr-OSD-CopyOSDLogToFileShare/releases/tag/v1.0.0)
...
Use the [v1.0.0 release](https://github.com/vartaxe/ConfigMgr-OSD-CopyOSDLogToFileShare/releases/tag/v1.0.0) for the versioned ZIP and SHA-256 sidecar, and read the [release notes](RELEASE-NOTES.md#v100) for its changes. Each release keeps its own identity; later changes do not replace earlier downloads. This site follows `main`, so its documentation can be newer than a downloaded package.

A regular GitHub release distributes a reviewed version; it does not establish live platform compatibility. Check the [validation scope](docs/validation.md#what-each-result-establishes) and [exit-code contract](docs/logging.md#exit-codes) before rollout.

The workflow initially publishes a prerelease candidate. Regular/latest status follows only after downloaded-asset verification and explicit maintainer approval to promote the same release; see the [two-stage release process](docs/release-process.md#publish-only-after-approval).

MIT licensed by Claudio Mendes. See the [license](LICENSE) and [security reporting guidance](SECURITY.md#reporting).
