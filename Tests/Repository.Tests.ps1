...
Describe 'Repository contract' {
    It 'keeps one matching three-part numeric version in the script and manifest' {
        ...
        $script:ProductionScript | Should -Match ('(?m)^Version:\s+' + [regex]::Escape($ManifestVersion) + '\r?$')
    }
    ...
    It 'has one matching SHA-256 entry for every maintained file except the checksum manifest' {
        $Names = @()
        $Lines = @(Get-Content -LiteralPath (Join-Path $script:Root 'CHECKSUMS.txt'))
        ($Lines -join "`n") | Should -BeExactly (($Lines | Sort-Object) -join "`n")
    ...
    It 'pins checkout actions, retains publisher verification, and preserves release gates' {
        ...
        $ReleaseWorkflow | Should -Not -Match '--latest'

        $ReleaseDocumentation = Get-Content -LiteralPath (Join-Path $script:Root 'docs\release-process.md') -Raw
        foreach ($RequiredText in 'Stage 1: Publish the prerelease candidate', 'Stage 2: Verify and approve promotion',
            'Download', 'same release ID', 'make_latest', 'After explicit approval') {
            $ReleaseDocumentation | Should -Match ([regex]::Escape($RequiredText))
        }
