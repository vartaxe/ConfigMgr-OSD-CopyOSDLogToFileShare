...
Describe 'Runtime behavior' {
    BeforeEach {
        ...
        $script:Version = (Get-Content -LiteralPath (Join-Path $PSScriptRoot '..\VERSION') -Raw).Trim()
    ...
    Describe 'Collection and archive behavior' {
        ...
        It 'writes a real ZIP containing the manifest and collected content' {
            ...
            $Manifest.Version | Should -BeExactly $script:Version
            @($Manifest.Items).Count | Should -Be 0
        }

        It 'preserves an existing ZIP when the replacement source exists: <SourceExists>' -Tag Regression -TestCases @(
            @{ SourceExists = $true }, @{ SourceExists = $false }
        ) {
            param($SourceExists)
            $Root = Join-Path $TestDrive ([guid]::NewGuid().ToString('N'))
            $Content = Join-Path $Root 'content'
            [void](New-Item -Path $Content -ItemType Directory -Force)
            Set-Content -LiteralPath (Join-Path $Content 'original.log') -Value 'retained diagnostic'
            $Archive = Join-Path $Root 'retained.zip'
            ...
            if ($SourceExists) {
                Set-Content -LiteralPath (Join-Path $Content 'new.log') -Value 'new diagnostic'
            }
            else {
                $Content = Join-Path $Root 'missing'
            }

            { Invoke-ArchiveCreation -SourceFolder $Content -ArchivePath $Archive } | Should -Throw

            (Get-FileHash -LiteralPath $Archive -Algorithm SHA256).Hash | Should -BeExactly $OriginalHash
        }
    ...
    Describe 'TCP preflight resource handling' {
        It 'closes acquired resources after <Outcome> without making a network connection' -TestCases @(
            @{ Outcome = 'success'; Events = 'begin,wait,end,client-close,wait-close' },
            @{ Outcome = 'timeout'; Events = 'begin,wait,client-close,wait-close' },
            @{ Outcome = 'connect failure'; Events = 'begin,wait,end,client-close,wait-close' },
            @{ Outcome = 'start failure'; Events = 'begin,client-close' }
        ) {
            param($Outcome, $Events)
            $script:TcpOutcome = $Outcome
            $script:TcpEvents = New-Object System.Collections.Generic.List[string]
            $script:ObservedTcpTimeout = $null
            $Handle = [pscustomobject]@{}
            $Handle | Add-Member ScriptMethod WaitOne {
                $script:TcpEvents.Add('wait')
                $script:ObservedTcpTimeout = $args[0]
                return $script:TcpOutcome -ne 'timeout'
            }
            $Handle | Add-Member ScriptMethod Close { $script:TcpEvents.Add('wait-close') }
            $script:TcpAsyncResult = [pscustomobject]@{ AsyncWaitHandle = $Handle }
            $script:TcpClient = [pscustomobject]@{}
            $script:TcpClient | Add-Member ScriptMethod BeginConnect {
                $script:TcpEvents.Add('begin')
                if ($script:TcpOutcome -eq 'start failure') {
                    throw [InvalidOperationException]::new('Connection could not start')
                }
                return $script:TcpAsyncResult
            }
            $script:TcpClient | Add-Member ScriptMethod EndConnect {
                $script:TcpEvents.Add('end')
                if ($script:TcpOutcome -eq 'connect failure') {
                    throw [IO.IOException]::new('Connection failed')
                }
            }
            $script:TcpClient | Add-Member ScriptMethod Close { $script:TcpEvents.Add('client-close') }
            Mock New-Object { $script:TcpClient } -ParameterFilter { $TypeName -eq 'System.Net.Sockets.TcpClient' }

            $ComputerName = 'unused.invalid'
            Test-TcpPort -ComputerName $ComputerName -Port 445 -TimeoutSeconds 7 |
                Should -Be ($Outcome -eq 'success')

            ($script:TcpEvents -join ',') | Should -BeExactly $Events
            if ($Outcome -ne 'start failure') {
                $script:ObservedTcpTimeout | Should -Be ([TimeSpan]::FromSeconds(7))
            }
        }
    }
    ...
    Describe 'SMB inspection policy' {
        ...
        It 'preserves the safe inspection failure category for <Failure> in upload diagnostics' -Tag Regression -TestCases @(
            @{ Failure = 'ambiguous identity' }, @{ Failure = 'inspection error' }
        ) {
            param($Failure)
            if ($Failure -eq 'ambiguous identity') {
                $script:Connection.Credential = 'CONTOSO\someone-else'
            }
            else {
                Mock Get-SmbConnection { throw [InvalidOperationException]::new('test-secret-only raw inspection failure') }
            }
            $SafeMessage = ''
            try {
                Confirm-SmbConnectionSecurity @script:Inspection
            }
            catch {
                $SafeMessage = Get-SafeErrorMessage -ErrorRecord $_
            }

            $SafeMessage | Should -BeExactly 'Unable to verify SMB connection security.'
            $SafeMessage | Should -Not -Match 'test-secret-only|someone-else|raw inspection failure'
        }
    ...
    Describe 'SMB connection controls and upload' {
        BeforeEach {
            ...
            foreach ($Name in @('BlockNTLM', 'RequireIntegrity', 'RequirePrivacy', 'Credential')) {
                if ($script:InboxMappingCommand.Parameters.ContainsKey($Name)) {
        ...
        It 'builds arguments supported by one real inbox mapping parameter set' {
            ...
            if (-not $Command.Parameters.ContainsKey('BlockNTLM')) {
        ...
        It 'clears credential-bearing mapping options after a <Failure> preflight failure' -Tag Regression -TestCases @(
            @{ Failure = 'TCP' }, @{ Failure = 'inspection' }
        ) {
            param($Failure)
            $script:ObservedMappingOptions = @{
                RemotePath = '\\fileserver.contoso.com\OSDLogs$'
                UserName   = $script:Credential.UserName
                Password   = $script:Credential.GetNetworkCredential().Password
            }
            Mock Get-SmbMappingOption { $script:ObservedMappingOptions }
            if ($Failure -eq 'TCP') {
                Mock Test-TcpPort { $false }
            }
            else {
            ...
            { Send-Archive @script:Upload } | Should -Throw

            $script:ObservedMappingOptions.Count | Should -Be 0
            Should -Invoke New-SmbMapping -Times 0 -Exactly
    ...
    Describe 'Orchestration with mocked Task Sequence and network' {
        ...
        It 'does not remove archives or staging retained by an earlier run after a later successful upload' {
            $EarlierRun = Join-Path $script:Pending 'earlier-run'
            [void](New-Item -Path $EarlierRun -ItemType Directory -Force)
            Set-Content -LiteralPath (Join-Path $EarlierRun 'retained.log') -Value 'earlier diagnostic'
            $EarlierArchive = Join-Path $script:Pending 'earlier-run.zip'
            [IO.Compression.ZipFile]::CreateFromDirectory($EarlierRun, $EarlierArchive)
            $EarlierHash = (Get-FileHash -LiteralPath $EarlierArchive -Algorithm SHA256).Hash

            & $script:MainBody

            $script:ObservedExitCode | Should -Be 0
            (Get-FileHash -LiteralPath $EarlierArchive -Algorithm SHA256).Hash | Should -BeExactly $EarlierHash
            Get-Content -LiteralPath (Join-Path $EarlierRun 'retained.log') | Should -BeExactly 'earlier diagnostic'
            @(Get-ChildItem -LiteralPath $script:Pending).Count | Should -Be 2
        }
        It 'retains staging and does not attempt upload when ZIP creation fails' {
            Mock Invoke-ArchiveCreation { throw [IO.IOException]::new('Archive creation failed') }

            & $script:MainBody

            $script:ObservedExitCode | Should -Be 1
            @(Get-ChildItem -LiteralPath $script:Pending -Filter Manifest.json -Recurse).Count | Should -Be 1
            Should -Invoke Send-Archive -Times 0 -Exactly
        }
