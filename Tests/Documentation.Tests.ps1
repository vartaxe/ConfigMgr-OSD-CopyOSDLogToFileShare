...
Describe 'Documentation and Pages contract' {
    ...
    It 'requests diagnostic versions and private reporting in <Name>' -Tag 'MaintenanceRegression' -ForEach @(
        ...
        $Content = Get-Content -LiteralPath (Join-Path $script:DocumentationRoot ".github\ISSUE_TEMPLATE\$Name") -Raw
    ...
    It 'asks bug reporters for reproduction steps and an exit code' -Tag 'MaintenanceRegression' {
        $Content = Get-Content -LiteralPath (Join-Path $script:DocumentationRoot '.github\ISSUE_TEMPLATE\bug_report.yml') -Raw
