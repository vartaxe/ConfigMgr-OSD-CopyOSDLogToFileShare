...
4. Finish all maintained file changes and regenerate `CHECKSUMS.txt` last using the [checkout convention](#checksum-checkout-convention). `-SkipChecksums` skips only the validator's final manifest phase; repository checksum tests still run, so regenerate before complete validation.
