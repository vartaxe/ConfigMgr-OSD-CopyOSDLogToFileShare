...
GitHub Pages publishes `main` from the repository root using Cayman and the shared layout. `index.md` is a focused landing page rather than a README include. Markdown guides use relative source links; `jekyll-relative-links` rewrites those to their generated page URLs, and `jekyll-optional-front-matter` renders policy pages without requiring YAML headers.
...
After changing site content, check the real Pages build and rendered light/dark desktop/mobile pages, including keyboard focus and the skip link. Local Jekyll builds require Ruby and the GitHub Pages dependencies; no separate site-build workflow or runtime dependency is required for branch-based Pages.
