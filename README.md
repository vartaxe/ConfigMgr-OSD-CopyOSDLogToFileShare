...
Live ConfigMgr, WinPE, and SMB validation remains pending. A regular GitHub release is a distribution choice, not proof of live compatibility. Run the short [environment rollout checklist](docs/validation.md#required-live-tests) before broad deployment; CI badges describe workflow state only.

The workflow publishes a prerelease candidate. Downloaded-asset verification and
maintainer approval precede promotion to a regular release marked latest; see the
[release process](docs/release-process.md). Release status is not platform certification.
