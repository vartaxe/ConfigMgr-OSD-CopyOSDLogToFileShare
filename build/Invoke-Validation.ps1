...
Skips only the final manifest verification phase. Repository checksum tests still run.
#>
[CmdletBinding()]
param(
    [string]$Tag,
...
try {
    ...
    if ($null -eq $ScriptAst) {
        throw 'Production script is missing.'
    }
    ...
    if ($RepositoryVersion -cne $ScriptVersion) {
        throw 'VERSION and script version differ.'
    }
    ...
    if (-not $SkipChecksums) {
        ...
        Write-Warning 'Checksums skipped for developer loop; not final validation.'
    }
